#!/usr/bin/env python3
"""
DATA CLEANING PIPELINE - CLEAN REGIME 2025 (POST-REVERT) - FIXED
Extract and clean all trading data for external verification
Using Docker database connection with correct schema
"""
...
class DataCleaner2025:
    ...
    def extract_trades_data(self):
        """Extract trades data using Docker with correct schema"""
        ...
        try:
            ...
            cmd = [
                "docker", "exec", "-i", "trading_db",
                "psql", "-U", "trading_user", "-d", "trading_db",
                "-c", "COPY (SELECT id, trade_id, entry_time, exit_time, symbol, instrument_type, strike_price, expiry_date, entry_price, exit_price, quantity, gross_pnl, brokerage, taxes, net_pnl, pnl_percentage, strategy_name, strategy_weight, signal_strength, ml_score, model_version, model_hash, features_snapshot, spot_price_entry, iv_entry, vix_entry, pcr_entry, market_regime_entry, regime_confidence, entry_hour, entry_minute, day_of_week, is_expiry_day, days_to_expiry, oi_entry, volume_entry, bid_entry, ask_entry, spread_entry, oi_exit, volume_exit, bid_exit, ask_exit, spread_exit, spot_price_exit, iv_exit, vix_exit, pcr_exit, market_regime_exit, exit_hour, exit_minute, delta_entry, gamma_entry, theta_entry, vega_entry, delta_exit, gamma_exit, theta_exit, vega_exit, target_price, stop_loss_price, max_profit_reached, max_loss_reached, status, is_winning_trade, hold_duration_minutes, signal_reason, exit_reason, notes, tags, risk_amount, risk_reward_ratio, position_size_pct FROM trades ORDER BY entry_time) TO STDOUT WITH (FORMAT csv, HEADER);"
            ]
            ...
            if result.returncode == 0 and result.stdout:
                ...
                if not df.empty:
                    # Extract SAC weights from strategy_weight column
                    df['sac_allocation'] = df['strategy_weight'] / 100.0  # Convert percentage to decimal
